...
import { TasksContext } from "../context/TasksContext"; // Ensure this context is correctly defined and imported
...
const UpdateTask = ({ setIsModalOpen }) => {
    ...
    useEffect(() => {
        if (Object.keys(selectTask).length !== 0) {
            setModelTitle(selectTask.title)
            setModelDescription(selectTask.des)
            setId(selectTask.id)
        }
    }, [selectTask])
    ...
    const updateTask = () => {
        ...
        // Example action dispatch to update the task
        dispatch({
            type: 'UPDATE_TASK',
            payload: {
                title: modelTitle,
